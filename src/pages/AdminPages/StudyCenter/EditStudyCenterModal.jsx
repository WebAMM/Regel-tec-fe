...
import { useEditStudyCenterMutation } from "../../../api/apiSlice";
import { toast } from "react-toastify";
import Loader from "../../../components/Loader";

const editStudyCenterValidation = Yup.object({
...
const EditStudyCenterModal = ({ open, onClose, studyCenter }) => {
  const [editStudyCenter, { isLoading }] = useEditStudyCenterMutation();
  console.log("studyCenter", studyCenter);
  if (!open || !studyCenter) return null;

  const initialValues = {
    name: studyCenter.name || "",
    address: studyCenter.address || "",
    phone: studyCenter.contactNumber || "",
    email: studyCenter.email || [],
    city: studyCenter.city || "",
    state: studyCenter.state || "",
    zipCode: studyCenter.zipCode || "",
    searchRadius: studyCenter.searchRadius || "",
  };
  ...
  const handleSubmit = async (values) => {
    try {
      const payload = {
        ...
        phone: values.phone,
        email: values.email.filter((e) => e.trim() !== ""),
        city: values.city,
        state: values.state,
        searchRadius: Number(values.searchRadius),
      };
      await editStudyCenter(payload).unwrap();
      toast.success("Study Center Updated Successfully");
      onClose();
    } catch (error) {
      toast.error(error?.data?.message || "Failed to update Study Center");
    }
  };
    ...
    <div className="fixed inset-0 bg-[#00000085] z-50 flex items-center justify-center">
      <div className="bg-white rounded-xl w-full max-w-4xl p-6 shadow-lg max-h-[90vh] overflow-y-auto">
        ...
        <Formik
          ...
          validationSchema={editStudyCenterValidation}
          enableReinitialize
        >
...
export default EditStudyCenterModal;
